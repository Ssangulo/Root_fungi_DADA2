...
kable(tab_s7, digits = 3,
      caption = "Table S7. Linear model results for centroid-based turnover.")
...
## Diversity trajectories across sites: testing parallelism for TD and PD
...
# Inputs
ps_ind  <- individual_ps   # collapsed replicates 
...
p_div_traj <- ggplot(div_long, aes(x = habitat, y = estimate, color = site)) +
  ...
  geom_point(
    data = obs_summary,
    aes(x = habitat, y = obs_mean, color = site),
    shape = 21,
    fill = "white",
    stroke = 0.9,
    size = 2.2,
  ...
  labs(
    ...
    title = "Per-sample TD and PD trajectories across sites",
    subtitle = "Raw sample values, observed means (hollow points), and bias-adjusted factor-model means with 95% CI"
  ) +
...
# Optional export
write.csv(model_screen, file.path(out_tab_dir, "Table_S7b_model_family_screening.csv"), row.names = FALSE)
write.csv(as.data.frame(anova_td_factor), file.path(out_tab_dir, "Table_S7c_div_parallelism_TD_factor_anova.csv"), row.names = FALSE)
write.csv(as.data.frame(anova_pd_factor), file.path(out_tab_dir, "Table_S7d_div_parallelism_PD_factor_anova.csv"), row.names = FALSE)
write.csv(as.data.frame(anova_td_num), file.path(out_tab_dir, "Table_S7e_div_parallelism_TD_numeric_anova.csv"), row.names = FALSE)
write.csv(as.data.frame(anova_pd_num), file.path(out_tab_dir, "Table_S7f_div_parallelism_PD_numeric_anova.csv"), row.names = FALSE)
write.csv(as.data.frame(slopes_td), file.path(out_tab_dir, "Table_S7l_div_parallelism_TD_slopes.csv"), row.names = FALSE)
write.csv(as.data.frame(slopes_pd), file.path(out_tab_dir, "Table_S7m_div_parallelism_PD_slopes.csv"), row.names = FALSE)
write.csv(as.data.frame(emm_td), file.path(out_tab_dir, "Table_S7g_div_parallelism_TD_emmeans.csv"), row.names = FALSE)
write.csv(as.data.frame(emm_pd), file.path(out_tab_dir, "Table_S7h_div_parallelism_PD_emmeans.csv"), row.names = FALSE)
write.csv(as.data.frame(contr_td), file.path(out_tab_dir, "Table_S7i_div_parallelism_TD_pairwise_habitat.csv"), row.names = FALSE)
write.csv(as.data.frame(contr_pd), file.path(out_tab_dir, "Table_S7j_div_parallelism_PD_pairwise_habitat.csv"), row.names = FALSE)
write.csv(meta, file.path(out_tab_dir, "Table_S7k_per_sample_TD_PD_values.csv"), row.names = FALSE)
...
## 10) Figure S10: diagnostics for NTI_aw model
diagdf <- data.frame(
