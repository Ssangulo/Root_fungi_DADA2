...
All figures (PNG) and result tables (CSV) are stored within the project repository using relative paths. Rendering this document therefore requires cloning the repository and opening the Quarto file from the repository root.

**Data availability.**

Repository: <https://github.com/root-fungi/Root_fungi_DADA2/tree/main>\
...
ggsave(
  filename = file.path(out_fig_dir, "Fig_4_iNEXT3D_TD.png"),
  plot = p_TD, width = 10, height = 6, dpi = 800, bg = "white"
)

# ---- Table S10 (TD summary): use TDAsyEst as your “Observed/Asymptotic/CI/Coverage” table ----
...
write.csv(
  tab_s10,
  file.path(out_tab_dir, "Table_S10_iNEXT3D_taxonomic_diversity.csv"),
  row.names = FALSE
)
...
ggsave(
  filename = file.path(out_fig_dir, "Fig_S12_iNEXT3D_PD.png"),
...
write.csv(
  tab_s11,
  file.path(out_tab_dir, "Table_S11_iNEXT3D_phylogenetic_diversity.csv"),
  row.names = FALSE
)
```
